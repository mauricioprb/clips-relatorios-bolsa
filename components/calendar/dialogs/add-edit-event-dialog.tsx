...
export function AddEditEventDialog({ children, startDate, startTime, event }: IProps) {
  ...
  const fallbackUser = event?.user ||
    users?.[0] || { id: "user-1", name: "Bolsista", picturePath: null };
  ...
  const onSubmit = (values: TEventFormData) => {
    try {
      const formattedEvent: IEvent = {
        ...
        id: isEditing && event?.id ? event.id : generateEventId(),
        user: fallbackUser,
        color: values.color,
      ...
      toast.error(`Não foi possível ${isEditing ? "editar" : "criar"} o evento`);
    }
  };
