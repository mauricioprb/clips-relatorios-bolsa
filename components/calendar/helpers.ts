...
export function rangeText(view: TCalendarView, date: Date): string {
  ...
  return `${format(start, FORMAT_STRING, { locale: ptBR })} - ${format(end, FORMAT_STRING, {
    locale: ptBR,
  })}`;
}
...
export const getColorClass = (color: string): string => {
  const colorClasses: Record<TEventColor, string> = {
    ...
    roxo: "border-purple-200 bg-purple-50 text-purple-700 dark:border-purple-800 dark:bg-purple-950 dark:text-purple-300",
    cinza:
